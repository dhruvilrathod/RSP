...
def parse_input(text):
    p, t = text.split("\n\n")
    return p.split(", "), t.split("\n")

def can_make_design(towel, patterns):
...
def total_ways_to_make_towel(towel, patterns):
    dp = [0] * (len(towel) + 1)
    dp[0] = 1
...
def count_possible_towels(text):
    patterns, towels = parse_input(text)
    count = 0
    for towel in towels:
        if can_make_design(towel, patterns):
            count += 1
    return count

def count_possible_ways(text):
    patterns, towels = parse_input(text)
    count = 0
    for towel in towels:
        count += total_ways_to_make_towel(towel, patterns)
    return count

total = count_possible_towels(text)
print("total words: ", total)

total1 = count_possible_ways(text)
print("total possibility: ", total1)