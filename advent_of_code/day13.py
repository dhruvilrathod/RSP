...
def parse_text(text):
    ...
    for i in machines:
        ...
        for l in range(len(lines)):
            if l <2:
                eqs = lines[l].split(": ")[1].split(", ")
                x = eqs[0].split("+")[1]
                y = eqs[1].split("+")[1]
            else:
                poss = lines[l].split(": ")[1].split(", ")
                x= poss[0].split("=")[1]
                y= poss[1].split("=")[1]
            m.append(int(x))
            m.append(int(y))
...
def solve_machine(machine):
    ...
    for a_presses, b_presses in product(range(max_presses + 1), repeat=2):
        move_x = a_presses * a_x + b_presses * b_x
        move_y = a_presses * a_y + b_presses * b_y

        if move_x == p_x and move_y == p_y:
...
def parse_text_with_error(text):
    parsed = []
    machines = text.split("\n\n")
    for i in machines:
        m = []
        ...
        for l in range(len(lines)):
            if l <2:
                ...
                poss = lines[l].split(": ")[1].split(", ")
                x= int(poss[0].split("=")[1]) + 10000000000000
                y= int(poss[1].split("=")[1]) + 10000000000000
            m.append(int(x))
            m.append(int(y))
        parsed.append(m)
...
def solve_machine_large(machine):
    a_x, b_x, a_y, b_y, p_x, p_y = machine

    transform_x1, transform_x2 = a_x * b_y, b_x * a_y
    transform_z1, transform_z2 = p_x * b_y, p_y * a_y
    ...
    b = (p_x - a * a_x) // a_y if (p_x - a * a_x) % a_y == 0 else -1

    return (a, b)


def min_tokens_to_win_new(text):

    machines = parse_text_with_error(text)
    total_tokens = 0
    prizes_won = 0

    for machine in machines:
        a,b = solve_machine_large(machine)
        if a >= 0 and b >= 0:
            prizes_won += 1
            total_tokens += a * 3 + b
...
tokens = min_tokens_to_win(text)
print("min token = ", tokens)

new_tokens = min_tokens_to_win_new(text)
print("min token updated = ", new_tokens)